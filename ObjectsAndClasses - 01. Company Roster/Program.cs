    ...
    class Program
    {
        static void Main()
        {
            ...
            for (int i = 0; i < numberOfEmployees; i++)
            {
                ...
                if (!departments.Any(d => d.DepartmentName == dataEntries[2]))
                {
                    departments.Add(new Department(dataEntries[2]));
                }

                departments.Find(d => d.DepartmentName == dataEntries[2]).AddNewEmployee(dataEntries[0], decimal.Parse(dataEntries[1]));
